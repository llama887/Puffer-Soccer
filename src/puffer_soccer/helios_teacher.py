...
FIELD_HALF_Y = 35.0
GOAL_HALF_Y = 20.0
BALL_CONTROL_RADIUS = 4.172818827132517
BALL_CHASE_MARGIN = 3.844064639178816
TARGET_ANGLE_TOLERANCE = 0.501127891779688
KICK_ANGLE_TOLERANCE = 0.4362070926344311
BALL_DECAY = 0.85
MAX_BALL_SPEED = 5.0
...
class HeliosTeacherConfig:  # pylint: disable=too-many-instance-attributes
    ...
    space. The default values are intentionally non-round because they are the best robust
    configuration from local Optuna trials against the current best checkpoint on June 14, 2026.
    Keeping them in one immutable config lets future experiments sweep the approximation while
    leaving the evaluation script and possible imitation-learning code unchanged.
    """
    ...
    dribble_kick_index: int = 2
    receiver_reach_buffer: float = 0.6910618972159475
    opponent_reach_buffer: float = 0.2910074132584326
    defensive_block_distance: float = 8.294119932858992
...
class HeliosTeacher:  # pylint: disable=too-few-public-methods
    ...
    def _goalie_target(self, context: TeamContext) -> np.ndarray:
        ...
        ball, bias toward the near post when the ball is wide, and react to the ball's current
        velocity when a direct shot is already traveling toward goal. This target is separate
        from ordinary role movement so goalie behavior does not become just another defender slot.
        """

        local_ball_x = float(context.ball_xy[0] * context.attack_direction)
        local_ball_y = float(context.ball_xy[1])
        local_ball_vx = float(context.ball_velocity[0] * context.attack_direction)
        local_ball_vy = float(context.ball_velocity[1])
        own_goal_x = -context.half_x + 2.0
        y_from_angle = 0.45 * local_ball_y
        if local_ball_vx < -0.05:
            steps_to_goal = max(0.0, (own_goal_x - local_ball_x) / local_ball_vx)
            predicted_y = local_ball_y + local_ball_vy * steps_to_goal
            if abs(predicted_y) <= GOAL_HALF_Y + 4.0:
                y_from_angle = 0.80 * predicted_y
